...
final class RevenueCodeFeeCatalogVisibilityScenario
{
    ...
    public function prepare(LifecycleScenarioDefinition $scenario, string $runId, array $actors, ScenarioArtifactStore $artifactStore): array
    {
        ...
        $steps = [
            ...
            $this->step('provision-coverage-recorded', 'Verify provision coverage is distinct from executable policy', ['provision_count' => 81, 'reconciliation_required_count' => 80], ['provision_count' => RevenueCodeProvision::query()->count(), 'reconciliation_required_count' => RevenueCodeProvision::query()->where('reconciliation_status', 'reconciliation_required')->count()]),
            ...
            $this->step('policy-boundary-clauses-recorded', 'Record non-schedule legal clauses without authorizing financial execution', ['provision_count' => 79, 'clause_count' => 425, 'reconciliation_required_count' => 425, 'ceiling_count' => 5, 'execution_ready_count' => 0], $policyBoundarySummary),
        ];
