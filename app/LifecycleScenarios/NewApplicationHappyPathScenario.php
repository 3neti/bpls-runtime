...
final class NewApplicationHappyPathScenario
{
    ...
    public function run(): array
    {
        ...
        return DB::transaction(function (): array {
            ...
            $application = $this->checkpoint('Brand-new Citizen did not establish and lodge the first canonical owner, business, and New Application', function () use ($actors, $linesOfBusiness): PermitApplication {
                $citizen = $actors['citizen']->fresh();
                $this->assert($citizen->business_owner_id === null, 'Scenario 02 Citizen did not start without a BusinessOwner.');
                $this->assert($citizen->permitApplications()->count() === 0, 'Scenario 02 Citizen did not start without applications.');
                ...
                $draft = $this->createCitizenDraft->handle([
                    'owner_name' => 'Scenario 02 Synthetic Owner',
                    'owner_email' => null,
                    'owner_phone' => null,
                    'owner_address' => 'Synthetic Ipil product laboratory address',
                    'business_name' => 'Scenario 02 Market and Kitchen',
                    'trade_name' => 'Scenario 02 New Application Laboratory',
                    'registration_number' => 'S02-NEW-APPLICATION-HAPPY-PATH',
                ...
                $this->assert($draft->status === PermitApplicationStatus::Draft, 'Canonical Citizen intake did not preserve the draft boundary.');
                $this->assert($draft->submitted_at === null, 'Citizen draft was incorrectly marked submitted.');
                $this->assert($draft->application_number === null, 'Citizen draft manufactured an unresolved official application number.');
                ...
                $metadata['lifecycle_scenario'] = [
                    'id' => NewApplicationHappyPathDefinition::Id,
                    'run_id' => NewApplicationHappyPathDefinition::RunId,
                    'definition_revision' => NewApplicationHappyPathDefinition::Revision,
                ...
                $metadata['business_permit_evaluation'] = [
                    ...
                    'scenario_id' => NewApplicationHappyPathDefinition::Id,
                    'production_liability' => false,
                ];
                $application->forceFill(['metadata' => $metadata])->save();

                $this->assert($application->type === PermitApplicationType::New, 'Canonical intake did not preserve New Application type.');
                $this->assert($application->status === PermitApplicationStatus::Assessment, 'Canonical Citizen submission did not enter municipal assessment processing.');
                $this->assert($application->submitted_at !== null, 'Canonical Citizen submission did not record lodged/submitted time.');
                $this->assert($application->application_number === null, 'Scenario manufactured an unresolved official application number.');
                $this->assert($application->tracking_reference !== null, 'Citizen submission did not preserve its non-official tracking reference.');
                $this->assert($application->lines()->count() === 2, 'New Application did not preserve exactly two declared LOBs.');
                $this->assert($application->submitted_by_id === $citizen->id, 'Citizen-originated New Application does not retain the Citizen submission actor.');
            ...
            $this->checkpoint('Unresolved New-application fee candidates were not kept outside synthetic liability', function () use ($evaluation, $actors): void {
                foreach ([
                    [
                        'key' => 'policy.new-micro-industry-mayors-permit.not-applicable',
                        'code' => 'MRC-3A-02-NEW-MAYORS-PERMIT-MICRO',
                        'reason' => 'The synthetic Scenario 02 business is not asserted to be a micro-industry; the unresolved enterprise-scale rule remains outside this provisional UAT payable.',
                    ],
                    [
                        'key' => 'policy.business-registration-plate.not-applicable',
                        'code' => 'MRC-3A-05-BUSINESS-REGISTRATION-PLATE',
                        'reason' => 'The statutory ceiling is not an executable exact charge; Scenario 02 records no registration-plate liability and does not infer a price.',
                    ],
                ] as $boundary) {
            ...
            $this->checkpoint('Canonical department work did not resolve every responsibility', function () use ($evaluation, $actors): void {
                foreach ($this->definition->responsibilities() as $responsibility) {
                    ...
                    $this->completeResponsibility->handle(
                        ...
                        $version->sequence,
                        $version->fingerprint,
                        NewApplicationHappyPathDefinition::Id.':'.NewApplicationHappyPathDefinition::Revision.':'.$responsibility['key'].':complete',
                    );
                }
            ...
            $this->expectFailure(
                ...
                function () use ($evaluation, $actors): mixed {
                    ...
                    return $this->completeResponsibility->handle(
                        ...
                        $version->sequence,
                        $version->fingerprint,
                        NewApplicationHappyPathDefinition::Id.':'.NewApplicationHappyPathDefinition::Revision.':unauthorized-treasurer-mutation',
                    );
                },
                'belongs to',
            );
            ...
            $this->expectFailure(
                ...
                function () use ($evaluation, $actors): mixed {
                    ...
                    return $this->completeResponsibility->handle(
                        ...
                        $version->sequence,
                        $version->fingerprint,
                        NewApplicationHappyPathDefinition::Id.':'.NewApplicationHappyPathDefinition::Revision.':post-schedule-mutation',
                    );
                },
    ...
    private function result(PermitApplication $application): array
    {
        ...
        $this->assertCitizenOriginatedOwnership($actors['citizen'], $application);
        ...
        $this->assert($projection['total_amount_cents'] === NewApplicationHappyPathDefinition::ExpectedGrandTotalCents, 'Evaluation Grand Total does not equal PHP 1,220.00.');
        ...
        $this->assert(data_get($projection, 'financial_working_paper.grand_total_amount_cents') === NewApplicationHappyPathDefinition::ExpectedGrandTotalCents, 'Working-paper Grand Total is unavailable or incorrect.');
        ...
        return [
            ...
            'scenario_id' => NewApplicationHappyPathDefinition::Id,
            'scenario_revision' => NewApplicationHappyPathDefinition::Revision,
            'status' => 'passed',
            'business_question' => NewApplicationHappyPathDefinition::EvidenceQuestion,
            ...
            'onboarding' => [
                'canonical_action' => 'CreateCitizenPermitApplicationDraft + SubmitCitizenPermitApplication',
                'disposition' => 'Canonical Citizen intake establishes the first BusinessOwner and Business, creates the officially unnumbered New Application draft with LOB declarations, then formal Citizen submission enters municipal processing. Portal visibility follows the durable User to BusinessOwner to Businesses relationship; submitted_by_id remains the Citizen submission audit actor.',
                'empty_to_first_business' => [
            ...
            'isolation_inventory' => [
                'scenario_applications' => PermitApplication::query()
                    ->where('metadata->lifecycle_scenario->id', NewApplicationHappyPathDefinition::Id)
                    ->where('metadata->lifecycle_scenario->definition_revision', NewApplicationHappyPathDefinition::Revision)
                    ->count(),
                'scenario_businesses' => $application->business()->where('registration_number', 'S02-NEW-APPLICATION-HAPPY-PATH')->count(),
            ...
            'artifacts' => [
                'root' => 'lifecycle-scenarios/'.NewApplicationHappyPathDefinition::Id.'/'.NewApplicationHappyPathDefinition::RunId,
                'json' => 'result.json',
    ...
    private function existingApplication(): ?PermitApplication
    {
        ...
        if ($specimen instanceof LifecycleScenarioSpecimen) {
            if ($applications->count() !== 1 || $applications->first()?->id !== $specimen->permit_application_id) {
                throw new NewApplicationHappyPathFailure('Harness-owned persisted specimen is singular', 'Scenario 02 ownership manifest does not match exactly one application.');
            }
        ...
        if ($applications->isNotEmpty()) {
            throw new NewApplicationHappyPathFailure('Scenario persistence is harness-owned', 'Unowned Scenario 02 transactional residue exists; refusing name or metadata inference.');
        }
    ...
    private function assertCitizenOriginatedOwnership(User $citizen, PermitApplication $application): void
    {
        ...
        $this->assert($application->submitted_by_id === $citizen->id, 'Citizen-originated Scenario 02 does not retain the Citizen submission actor.');
        $this->assert($application->business->owner->businesses()->count() === 1, 'Scenario 02 Citizen does not own exactly the first newly established Business.');
        $this->assert($application->business->permitApplications()->count() === 1, 'Scenario 02 Business does not retain exactly the first New Application.');
    }
    ...
    private function actor(string $key, string $name, array $permissions): User
    {
        $role = Role::query()->firstOrCreate(
            ['code' => 'scenario-02-'.$key],
            ['name' => $name, 'description' => 'Synthetic Scenario 02 role; not a production municipal assignment.'],
        );
        ...
        $user = User::query()->firstOrCreate(
            ['email' => 'scenario-02-'.$key.'@example.test'],
            [
                'role_id' => $role->id,
                'name' => $name,
    ...
    private function linesOfBusiness(): array
    {
        return collect($this->definition->linesOfBusiness())->mapWithKeys(function (array $definition): array {
            ...
            $this->assert(data_get($lineOfBusiness->metadata, 'scenario_id') === NewApplicationHappyPathDefinition::Id, "LOB code [{$definition['code']}] is occupied by non-scenario data.");
    ...
    {
        $responsibilityKeys = collect($this->definition->responsibilities())->pluck('key');

        return array_values(collect($this->evaluationResolver->resolve($evaluation)['items'])
            ->filter(fn (array $item): bool => $responsibilityKeys->contains($item['key']))
    ...
    private function applicationEvaluationRouting(PermitApplication $application, array $linesOfBusiness): array
    {
        ...
        return [
            ...
            'source_facts' => ['lodged New Application', 'declared Lines of Business', 'Scenario 02 provisional UAT applicability', 'generated Evaluation responsibilities'],
            'classification' => 'provisional_uat',
            'groups' => $requiredWork
    ...
    private function responsibilityKeys(array $responsibilities): array
    {
        ...
        foreach ($responsibilities as $responsibility) {
            ...
            if (! is_string($key)) {
                throw new NewApplicationHappyPathFailure('Responsibility identity is present', 'A projected responsibility has no canonical key.');
            }
    ...
    private function routingKeys(array $routing): array
    {
        ...
        if (! is_array($requiredWork)) {
            throw new NewApplicationHappyPathFailure('Routing required work is present', 'The routing projection returned no required-work list.');
        }
        ...
        foreach ($requiredWork as $item) {
            ...
            if (! is_string($key)) {
                throw new NewApplicationHappyPathFailure('Routing identity is present', 'A routing requirement has no canonical key.');
            }
    ...
    private function timeline(): array
    {
        $steps = [
            ...
            ['citizen_started_empty', 'Citizen', 'Scenario precondition', 'Synthetic Citizen begins without a BusinessOwner, Business, or Permit Application.'],
            ['owner_onboarded', 'Citizen', 'CreateCitizenPermitApplicationDraft', 'Canonical Citizen intake establishes the first synthetic BusinessOwner.'],
            ['business_onboarded', 'Citizen', 'CreateCitizenPermitApplicationDraft', 'The same canonical draft action establishes the first Business owned by that BusinessOwner.'],
    ...
    private function lineSections(array $projection): array
    {
        ...
        if (! is_array($sections)) {
            throw new NewApplicationHappyPathFailure('Financial working paper has LOB sections', 'Evaluation projection did not return line sections.');
        }
    ...
    private function sectionCharges(array $section): array
    {
        ...
        if (! is_array($charges)) {
            throw new NewApplicationHappyPathFailure('Financial working paper has charge rows', 'A LOB section did not return charge rows.');
        }
    ...
    private function evaluationCharges(array $projection): array
    {
        ...
        if (! is_array($applicationCharges)) {
            throw new NewApplicationHappyPathFailure('Financial working paper has application-wide charges', 'Evaluation projection did not return application-wide charges.');
        }
    ...
    private function expectFailure(string $invariant, callable $operation, string $expectedMessage): array
    {
        try {
            ...
            if (! str_contains($exception->getMessage(), $expectedMessage)) {
                throw new NewApplicationHappyPathFailure($invariant, "Expected refusal containing [{$expectedMessage}], received [{$exception->getMessage()}].", $exception);
            }

            return ['passed' => true, 'message' => $exception->getMessage()];
        }

        throw new NewApplicationHappyPathFailure($invariant, 'The forbidden operation succeeded.');
    }
    ...
    private function checkpoint(string $invariant, callable $operation): mixed
    {
        try {
            return $operation();
        } catch (NewApplicationHappyPathFailure $failure) {
            throw $failure;
        } catch (Throwable $exception) {
            throw new NewApplicationHappyPathFailure($invariant, $exception->getMessage(), $exception);
        }
    }
    ...
    {
        if (! $condition) {
            throw new NewApplicationHappyPathFailure('Scenario 02 financial and lifecycle reconciliation', $detail);
        }
    }
}
