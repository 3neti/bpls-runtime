...
class LifecycleScenarioCommand extends Command
{
    ...
    public function handle(
        ...
        CitizenPermitAuthorityReviewVisibilityScenario $citizenPermitAuthorityReviewVisibilityScenario,
        ...
        try {
            ...
            if (in_array($phase, ['prepare', 'all'], true)) {
                ...
                $manifest = match ($scenario->key) {
                    ...
                    'citizen_new_permit_lifecycle_authority_boundary', 'nelson_walkthrough', 'new_permit_lifecycle_authority_boundary', 'stakeholder_preview_cycle_1' => $manualCollectionReceiptScenario->prepare($scenario, $runId, $actors, $artifactStore),
            ...
            if (in_array($phase, ['audit', 'all'], true)) {
                $manifest = match ($scenario->key) {
                    ...
                    'citizen_new_permit_lifecycle_authority_boundary', 'nelson_walkthrough', 'new_permit_lifecycle_authority_boundary', 'stakeholder_preview_cycle_1' => $manualCollectionReceiptScenario->audit($manifest ?? $this->requireManifest($artifactStore), $artifactStore),
