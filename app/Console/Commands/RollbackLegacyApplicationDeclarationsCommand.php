...
#[Signature('legacy:rollback-declarations
    {execution : Exact declaration mapping execution ID}
    {--rollback : Confirm accepted mappings and unchanged created lines may be removed}
    {--confirm-rollback : Second explicit rollback confirmation}
    {--json : Write only structured output}')]
#[Description('Rollback one completed declaration migration without deleting pre-existing or assessed lines.')]
class RollbackLegacyApplicationDeclarationsCommand extends Command
{
    public function handle(RollbackLegacyApplicationDeclarations $action): int
    {
        try {
            ...
            if (! is_int($executionId)) {
                throw new RuntimeException('The execution argument must be an exact positive declaration execution ID.');
            }

            $execution = $action->handle(LegacyDeclarationMappingExecution::query()->findOrFail($executionId));
        ...
        if ($this->option('json')) {
            ...
            $this->line('Pre-existing declaration lines deleted: no');
    ...
    private function writeEvidence(LegacyDeclarationMappingExecution $execution): string
    {
        ...
        $report = [
            'schema_version' => 'bpls.declaration-migration-rollback.v1',
            'execution_id' => $execution->id,
            'run_id' => $execution->run_reference,
            'status' => $execution->status->value,
            'removed_mapping_count' => $execution->metadata['rollback_mapping_count'] ?? 0,
            'deleted_created_line_count' => $execution->metadata['rollback_deleted_created_targets'] ?? 0,
            'remaining_execution_mappings' => $execution->mappings->count(),
            'pre_existing_targets_deleted' => false,
        ...
        if (! Storage::disk('local')->put($root.'/rollback.json', json_encode($report, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR)."\n")) {
            throw new RuntimeException('Declaration migration rollback evidence could not be written to private storage.');
        }
