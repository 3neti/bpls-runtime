...
#[Signature('legacy:rollback-applications
    {execution : Exact permit application mapping execution ID}
    {--rollback : Confirm accepted mappings and unchanged created applications may be removed}
    {--confirm-rollback : Second explicit rollback confirmation}
    {--json : Write only structured output}')]
#[Description('Rollback one completed permit application migration execution without deleting pre-existing or changed applications.')]
class RollbackLegacyPermitApplicationsCommand extends Command
{
    public function handle(RollbackLegacyPermitApplications $action): int
    {
        try {
            ...
            if (! is_int($executionId)) {
                throw new RuntimeException('The execution argument must be an exact positive permit application execution ID.');
            }

            $execution = $action->handle(LegacyApplicationMappingExecution::query()->findOrFail($executionId));
        ...
        if ($this->option('json')) {
            ...
            $this->line('Pre-existing applications deleted: no');
    ...
    private function writeEvidence(LegacyApplicationMappingExecution $execution): string
    {
        ...
        $report = [
            'schema_version' => 'bpls.application-migration-rollback.v1',
            'execution_id' => $execution->id,
            'run_id' => $execution->run_reference,
            'status' => $execution->status->value,
            'removed_mapping_count' => $execution->metadata['rollback_mapping_count'] ?? 0,
            'deleted_created_application_count' => $execution->metadata['rollback_deleted_created_targets'] ?? 0,
            'remaining_execution_mappings' => $execution->mappings->count(),
            'pre_existing_targets_deleted' => false,
        ...
        if (! Storage::disk('local')->put($root.'/rollback.json', json_encode($report, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR)."\n")) {
            throw new RuntimeException('Permit application migration rollback evidence could not be written to private storage.');
        }
