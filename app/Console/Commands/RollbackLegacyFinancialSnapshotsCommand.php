...
#[Signature('legacy:rollback-financial-snapshots
    {execution : Exact financial mapping execution ID}
    {--rollback : Confirm mappings and unchanged execution-created snapshots may be removed}
    {--confirm-rollback : Second explicit rollback confirmation}
    {--json : Write only structured output}')]
#[Description('Rollback one completed financial snapshot migration without deleting changed or collected records.')]
class RollbackLegacyFinancialSnapshotsCommand extends Command
{
    public function handle(RollbackLegacyFinancialSnapshots $action): int
    {
        try {
            ...
            if (! is_int($executionId)) {
                throw new RuntimeException('The execution argument must be an exact positive financial execution ID.');
            }

            $execution = $action->handle(LegacyFinancialMappingExecution::query()->findOrFail($executionId));
        ...
        if ($this->option('json')) {
            ...
            $this->line('Pre-existing financial records deleted: no');
    ...
    private function writeEvidence(LegacyFinancialMappingExecution $execution): string
    {
        ...
        $report = [
            'schema_version' => 'bpls.legacy-financial-snapshot-rollback.v1',
            'execution_id' => $execution->id,
            'run_id' => $execution->run_reference,
            'status' => $execution->status->value,
            'removed_mapping_count' => $execution->metadata['rollback_mapping_count'] ?? 0,
            'deleted_created_snapshot_count' => $execution->metadata['rollback_deleted_created_snapshots'] ?? 0,
            'remaining_execution_mappings' => $execution->mappings->count(),
            'pre_existing_targets_deleted' => false,
        ...
        if (! Storage::disk('local')->put($root.'/rollback.json', json_encode($report, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR)."\n")) {
            throw new RuntimeException('Financial snapshot migration rollback evidence could not be written to private storage.');
        }
