...
class PaidEstablishmentReportController extends Controller
{
    public function index(Request $request, BuildPaidEstablishmentsReport $report): Response
    {
        Gate::authorize(UserPermission::ViewReports->value);

        $payload = $report->handle($this->validatedFilters($request));

        return Inertia::render('reports/PaidEstablishments', [
    ...
    public function download(Request $request, BuildPaidEstablishmentsReport $report): StreamedResponse
    {
        Gate::authorize(UserPermission::ViewReports->value);

        $payload = $report->handle($this->validatedFilters($request));
        $filename = 'paid-establishments-'.$payload['filters']['year'].'.csv';
