...
{
    public function __construct(private PlanLegacyPermitEvidence $planner) {}
    ...
    public function handle(LegacyPermitEvidencePlan $plan, array $proposalIds, string $runReference): LegacyPermitEvidenceExecution
    {
        ...
        return DB::transaction(function () use ($plan, $proposalIds, $runReference, $selectionHash): LegacyPermitEvidenceExecution {
            ...
            $proposals = $lockedPlan->proposals()
                ->with(['legacyRecord', 'clearanceReconciliation'])
    ...
    private function assertExecutableSelection(Collection $proposals): void
    {
        foreach ($proposals as $proposal) {
            ...
            if ($proposal->kind !== 'clearance') {
                throw new RuntimeException("Permit-evidence proposal [{$proposal->id}] is not a pending clearance assignment and cannot execute.");
            }
            ...
            $this->validatedProjection($proposal);
            $this->applicationMapping($proposal);
        }
    }

    private function executeProposal(LegacyPermitEvidenceExecution $execution, LegacyPermitEvidenceProposal $proposal): string
    {
        $projection = $this->validatedProjection($proposal);
        $applicationMapping = $this->applicationMapping($proposal);
        ...
        LegacyPermitClearanceMapping::query()->create([
            ...
            'metadata' => [
                ...
                'projection_hash' => $proposal->projection_hash,
                'target_snapshot_hash' => $this->targetSnapshotHash($target),
                'completion_authority_asserted' => false,
            ],
        ]);
    ...
    private function validatedProjection(LegacyPermitEvidenceProposal $proposal): array
    {
        $record = $proposal->legacyRecord;
    ...
    private function applicationMapping(LegacyPermitEvidenceProposal $proposal): LegacyApplicationIdMapping
    {
        ...
        if (! $mapping->permitApplication()->exists()) {
            throw new RuntimeException("Permit-evidence proposal [{$proposal->id}] application mapping target is missing.");
        }
