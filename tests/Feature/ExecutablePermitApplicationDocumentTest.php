...
test('the same executable document progressively projects immutable assessment treasury and treasurer truth', function (): void {
    ...
    expect(data_get($document, 'declaration.state'))->toBe('frozen')
        ...
        ->and(data_get($document, 'page_2_assessment.total_label'))->toBe('Assessment total')
        ->and(data_get($document, 'page_2_assessment.total_source'))->toBe('assessment')
        ...
        ->and($pdf)->toContain('ASSESSMENT TOTAL PHP 1,220.00')
...
test('the executable html preserves the Ipil document nouns and mobile line grammar', function (): void {
    ...
    expect($create)->toContain('Application Form for Business Permit')
        ...
        ->and($create)->toContain('Fill blanks')
        ->and($create)->toContain('Legacy specimen')
        ->and($create)->toContain('Source specimen')
        ->and($create)->toContain('Load specimen')
        ->and($create)->toContain('permit-application-lab-details')
        ->and($create)->toContain('Specimen details')
        ->and($create)->not->toContain('Anything you have')
        ->and($create)->not->toContain('already entered stays unchanged')
        ...
        ->and($processingSheet)->toContain('document.page_2_assessment.total_label')
        ->and($processingSheet)->toContain('Assessment Reference')
        ->and($processingSheet)->toContain('post-payment-certification-summary')
        ->and($processingSheet)->toContain('offices certified')
        ->and($processingSheet)->toContain('Still required:')
        ->and($processingSheet)->toContain('Certification recorded in this Application:')
        ->and($processingSheet)->toContain('Certified & recorded')
        ->and($processingSheet)->toContain('scrollIntoView')
