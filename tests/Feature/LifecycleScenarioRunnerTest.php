...
function pendingPaymentReportsBrowserReport(array $manifest): array
{
    return [
        'assessment_summary' => [
        ...
        'collectibles' => [
            ...
            'total_amount_cents' => $manifest['resources']['collectibles_total_amount_cents'],
            'unscheduled_amount_cents' => $manifest['resources']['collectibles_unscheduled_amount_cents'],
            'csv_export_visible' => true,
            'mobile_visible' => true,
...
test('scenario registry discovers the role and permission matrix visibility scenario', function () {
    $scenario = app(LifecycleScenarioRegistry::class)->get('role_permission_matrix_visibility');
...
test('manual collection receipt scenario executes treasury actions idempotently', function () {
    ...
    expect($firstManifest['resources']['record_type'])->toBe('receipt')
        ...
        ->and($firstManifest['resources']['all_abstract_report_url'])->toBe('/staff/reports/all-abstract')
        ->and($firstManifest['resources']['all_abstract_status'])->toBe('blocked')
        ->and($firstManifest['resources']['all_abstract_can_generate'])->toBeFalse()
        ->and($firstManifest['resources']['all_abstract_can_export'])->toBeFalse()
        ->and($firstManifest['resources']['all_abstract_official_row_count'])->toBe(0)
        ->and($firstManifest['resources']['all_abstract_coverage_count'])->toBe(5)
        ->and($firstManifest['resources']['all_abstract_control_count'])->toBe(7)
        ->and($firstManifest['resources']['all_abstract_permit_collection_excluded'])->toBeTrue()
        ...
        ->and($firstManifest['resources']['plds_report_url'])->toBe('/staff/reports/plds')
        ->and($firstManifest['resources']['plds_status'])->toBe('blocked')
        ->and($firstManifest['resources']['plds_can_generate'])->toBeFalse()
        ->and($firstManifest['resources']['plds_can_export'])->toBeFalse()
        ->and($firstManifest['resources']['plds_official_row_count'])->toBe(0)
        ->and($firstManifest['resources']['plds_contract_column_count'])->toBe(23)
        ->and($firstManifest['resources']['plds_artifact_excluded'])->toBeTrue()
...
test('role permission matrix scenario preserves read-only canonical and browser evidence idempotently', function () {
    Storage::fake('local');

    $operator = configuredScenarioUser('operator@example.test');
    $scenario = app(LifecycleScenarioRegistry::class)->get('role_permission_matrix_visibility');
    $artifactStore = new ScenarioArtifactStore($scenario->key, 'role-permission-matrix-test-001');
    $runner = app(RolePermissionMatrixVisibilityScenario::class);

    $firstManifest = $runner->prepare($scenario, 'role-permission-matrix-test-001', [
        'operator' => $operator,
    ], $artifactStore);
    $secondManifest = $runner->prepare($scenario, 'role-permission-matrix-test-001', [
        'operator' => $operator,
    ], $artifactStore);
    $execution = $artifactStore->readJson('terminal/execution.json');
    ...
    expect($firstManifest['resources'])->toBe($secondManifest['resources'])
        ...
        ->and($execution['irreversible_actions'])->toBeFalse();

    $artifactStore->putJson('browser/report.json', [
        'result' => ['passed' => true],
        'role_permissions' => [
            'role_count' => $firstManifest['resources']['role_count'],
            'permission_count' => $firstManifest['resources']['permission_count'],
            'catalog_in_sync' => $firstManifest['resources']['catalog_in_sync'],
            'admin_override_role_count' => $firstManifest['resources']['admin_override_role_count'],
            'role_evidence' => $firstManifest['resources']['role_evidence'],
        ...
        'artifacts' => [
            'screenshots' => [
                '01-role-permission-matrix' => 'browser/screenshots/01-role-permission-matrix.png',
                '02-role-permission-matrix-mobile' => 'browser/screenshots/02-role-permission-matrix-mobile.png',
            ],
        ],
    ]);
...
test('revenue code fee catalog visibility scenario prepares deterministic catalog evidence idempotently', function () {
    ...
    expect($firstManifest['scenario']['key'])->toBe('revenue_code_fee_catalog_visibility')
        ...
        ->and($firstManifest['resources']['provision_count'])->toBe(108)
        ->and($firstManifest['resources']['reconciliation_required_count'])->toBe(107)
        ...
        ->and($firstManifest['resources']['policy_boundary_summary'])->toBe([
            'provision_count' => 106,
            'clause_count' => 593,
            'reconciliation_required_count' => 593,
            'ceiling_count' => 5,
            'execution_ready_count' => 0,
        ])
        ->and($firstManifest['resources']['policy_boundary_clause_codes'])->toHaveCount(593)
...
test('manual collection receipt scenario audit compares browser evidence with canonical treasury state', function () {
    ...
    $artifactStore->putJson('browser/report.json', [
        ...
        'reports' => [
            ...
            'all_abstract' => [
                ...
                'coverage_count' => 5,
                'control_count' => 7,
                'permit_collection_excluded' => true,
                'mobile_visible' => true,
                'mobile_horizontal_overflow' => false,
            ],
            'annex_c_dnfbp' => [
    ...
    expect($audited['result'])
        ...
        ->and(collect($artifactStore->readJson('terminal/audit.json')['checks'])->firstWhere('key', 'audit-browser-plds-authority-boundary')['passed'])->toBeTrue()
        ->and($artifactStore->exists('terminal/audit.json'))->toBeTrue()
        ->and($artifactStore->exists('summary.html'))->toBeTrue();
});
...
test('permit application pending payment scenario executes assessment and payment schedule actions idempotently', function () {
    ...
    expect($firstManifest['resources']['record_type'])->toBe('permit_application')
        ...
        ->and($firstManifest['resources']['collectibles_report_url'])->toContain('q=APP-SCENARIO-PERMIT-PENDING-PAYMENT-TEST-001')
        ->and($firstManifest['resources']['collectibles_business_name'])->toBe('Scenario Payment Business permit-pending-payment-test-001')
        ->and($firstManifest['resources']['collectibles_total_amount_cents'])->toBe(30_000)
        ->and($firstManifest['resources']['collectibles_unscheduled_amount_cents'])->toBe(30_000)
...
test('permit application pending payment scenario audit compares browser evidence with canonical state', function () {
    ...
    $artifactStore->putJson('browser/report.json', [
        ...
        'reports' => [
            ...pendingPaymentReportsBrowserReport($manifest),
