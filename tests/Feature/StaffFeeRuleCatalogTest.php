...
it('shows the read-only taxes and fees catalog to authorized staff', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('summary.provisions_recorded', 20)
            ->where('summary.provisions_requiring_reconciliation', 19)
            ->where('summary.provisions_linked_to_rules', 4)
            ->where('summary.policy_boundary_clauses', 87)
            ->where('summary.policy_boundary_clauses_requiring_reconciliation', 87)
            ->has('revenueCodeProvisions', 20)
            ...
            ->has('revenueCodePolicyBoundaries', 16)
            ...
            ->where('revenueCodePolicyBoundaries.7.provision.code', 'MRC-2E-01-BUSINESS-TAX-SCOPE')
            ->where('revenueCodePolicyBoundaries.7.clauses.2.clause_type', 'combined_tax_base')
            ->where('revenueCodePolicyBoundaries.8.provision.code', 'MRC-2E-02-03-ACCRUAL-PAYMENT')
            ->where('revenueCodePolicyBoundaries.8.clauses.2.code', 'MRC-2E-03-QUARTERLY-INSTALLMENTS')
            ->where('revenueCodePolicyBoundaries.9.provision.code', 'MRC-2E-04-A-C-PERMIT-RECEIPT-REQUIREMENTS')
            ->has('revenueCodePolicyBoundaries.9.clauses', 9)
            ->where('revenueCodePolicyBoundaries.9.clauses.7.clause_type', 'record_retention')
            ->where('revenueCodePolicyBoundaries.10.provision.code', 'MRC-2E-04-D-E-DECLARATIONS-DEFICIENCY')
            ->where('revenueCodePolicyBoundaries.10.clauses.5.clause_type', 'surcharge_interest')
            ->where('revenueCodePolicyBoundaries.11.provision.code', 'MRC-2E-04-DEATH-TAX-MAPPING')
            ->where('revenueCodePolicyBoundaries.11.clauses.0.clause_type', 'estate_continuation')
            ->where('revenueCodePolicyBoundaries.12.provision.code', 'MRC-2E-04-F-LOST-RECEIPT-CERTIFICATION')
            ->where('revenueCodePolicyBoundaries.12.clauses.0.amount_cents', 10000)
            ->where('revenueCodePolicyBoundaries.13.provision.code', 'MRC-2E-04-RETIREMENT')
            ->has('revenueCodePolicyBoundaries.13.clauses', 12)
            ->where('revenueCodePolicyBoundaries.13.clauses.10.clause_type', 'permit_cancellation')
            ->where('revenueCodePolicyBoundaries.14.provision.code', 'MRC-2E-04-G-LOCATION-TRANSFER')
            ->where('revenueCodePolicyBoundaries.14.clauses.0.clause_type', 'location_transfer')
            ->where('revenueCodePolicyBoundaries.15.provision.code', 'MRC-2F-01-PIL')
            ->has('revenueCodePolicyBoundaries.15.clauses', 29)
            ->where('revenueCodePolicyBoundaries.15.clauses.2.code', 'MRC-2F-01-PIL-GROCERY')
            ->where('revenueCodePolicyBoundaries.15.clauses.2.amount_cents', 616000000)
            ->where('revenueCodePolicyBoundaries.15.clauses.10.amount_cents', null)
            ->where('revenueCodePolicyBoundaries.15.clauses.28.clause_type', 'validation_fallback')
            ->has('feeRules.data', 4)
