...
it('shows the read-only taxes and fees catalog to authorized staff', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('summary.provisions_recorded', 15)
            ->where('summary.provisions_requiring_reconciliation', 14)
            ->where('summary.provisions_linked_to_rules', 4)
            ->where('summary.policy_boundary_clauses', 62)
            ->where('summary.policy_boundary_clauses_requiring_reconciliation', 62)
            ->has('revenueCodeProvisions', 15)
            ...
            ->has('revenueCodePolicyBoundaries', 11)
            ...
            ->where('revenueCodePolicyBoundaries.9.provision.code', 'MRC-2E-04-D-E-DECLARATIONS-DEFICIENCY')
            ->where('revenueCodePolicyBoundaries.9.clauses.5.clause_type', 'surcharge_interest')
            ->where('revenueCodePolicyBoundaries.10.provision.code', 'MRC-2F-01-PIL')
            ->has('revenueCodePolicyBoundaries.10.clauses', 29)
            ->where('revenueCodePolicyBoundaries.10.clauses.2.code', 'MRC-2F-01-PIL-GROCERY')
            ->where('revenueCodePolicyBoundaries.10.clauses.2.amount_cents', 616000000)
            ->where('revenueCodePolicyBoundaries.10.clauses.10.amount_cents', null)
            ->where('revenueCodePolicyBoundaries.10.clauses.28.clause_type', 'validation_fallback')
            ->has('feeRules.data', 4)
