...
it('shows the read-only taxes and fees catalog to authorized staff', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('revenueCodeScheduleMatrix.provision.code', 'MRC-2A-02-B-WHOLESALERS')
            ->where('revenueCodeScheduleMatrix.provision.linked_fee_rule_execution_status', 'blocked')
            ->where('revenueCodeScheduleMatrix.summary.row_count', 24)
            ->where('revenueCodeScheduleMatrix.summary.reconciliation_required_count', 3)
            ->where('revenueCodeScheduleMatrix.summary.overlap_count', 1)
            ->where('revenueCodeScheduleMatrix.summary.gap_count', 0)
            ->where('revenueCodeScheduleMatrix.summary.ceiling_count', 1)
            ->where('revenueCodeScheduleMatrix.summary.execution_ready', false)
            ->has('revenueCodeScheduleMatrix.rows', 24)
            ->where('revenueCodeScheduleMatrix.rows.7.code', 'MRC-2A-02-B-ROW-08')
            ->where('revenueCodeScheduleMatrix.rows.7.issues.0.type', 'overlap')
            ->where('revenueCodeScheduleMatrix.rows.17.normalization_status', 'reconciliation_required')
            ->where('revenueCodeScheduleMatrix.rows.23.is_ceiling', true)
            ->has('feeRules.data', 4)
