...
it('shows the read-only taxes and fees catalog to authorized staff', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('summary.policy_boundary_clauses', 12)
            ->where('summary.policy_boundary_clauses_requiring_reconciliation', 12)
            ...
            ->has('revenueCodePolicyBoundaries', 4)
            ->where('revenueCodePolicyBoundaries.0.provision.code', 'MRC-2A-02-C-EXPORTERS-ESSENTIALS')
            ->has('revenueCodePolicyBoundaries.0.clauses', 3)
            ->where('revenueCodePolicyBoundaries.0.clauses.0.code', 'MRC-2A-02-C-DEPENDENT-HALF-RATE')
            ->where('revenueCodePolicyBoundaries.0.clauses.0.is_ceiling', true)
            ->where('revenueCodePolicyBoundaries.0.clauses.0.candidate_values_are_non_executable', true)
            ->where('revenueCodePolicyBoundaries.1.provision.code', 'MRC-2A-02-D-RETAILERS')
            ->where('revenueCodePolicyBoundaries.1.clauses.1.rate_basis_points', '126.0000')
            ->where('revenueCodePolicyBoundaries.2.provision.code', 'MRC-2A-02-F-FINANCIAL-INSTITUTIONS')
            ->where('revenueCodePolicyBoundaries.2.clauses.1.clause_type', 'taxable_receipt_catalog')
            ->where('revenueCodePolicyBoundaries.3.provision.code', 'MRC-2A-02-H-PEDDLERS')
            ->where('revenueCodePolicyBoundaries.3.clauses.0.amount_cents', 6275)
            ->has('feeRules.data', 4)
